...
const usePersonStore = create<PersonStore>((set) => ({
    ...
    fetchPersons: async () => {
      try {
        const response = await axios.get('https://back-production-a5a2.up.railway.app');
    ...
    deletePerson: async (id: number) => {
      try {
        await axios.delete(`https://back-production-a5a2.up.railway.app/${id}`);
        set((state) => ({
...
const tableProps = {
  ...
  const handleSave = () => {
    form.validateFields().then((values) => {
      ...
      axios.post(`https://back-production-a5a2.up.railway.app`, newPerson)
          .then((response) => {
